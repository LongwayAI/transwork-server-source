...
import {
  formatTransworkCredits,
  parseTransworkCreditsToQuota,
  quotaToTransworkCredits,
} from '@/transwork/credits'
...
/**
 * Format quota into Gressio credits.
 * Quota is stored in units where `quotaPerUnit` equals 1 USD-equivalent.
 */
export function formatQuota(quota: number): string {
  return formatTransworkCredits(quota, {
    digitsLarge: 2,
    digitsSmall: 4,
    abbreviate: true,
  })
...
/**
 * Parse quota from user-facing credits back to quota units.
 * Kept under the historical name because several form models still use
 * *_dollars field names internally.
 */
export function parseQuotaFromDollars(amount: number): number {
  return parseTransworkCreditsToQuota(amount)
}

/**
 * Convert quota units to user-facing credits.
 * Reverse of parseQuotaFromDollars.
 */
export function quotaUnitsToDollars(units: number): number {
  return quotaToTransworkCredits(units)
}
