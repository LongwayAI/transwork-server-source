...
import {
  AdminSubscriptionLinkHeadCells,
  AdminSubscriptionLinkBodyCells,
  CancelStripeButton,
  useAdminSubscriptionLinks,
} from '@/transwork/subscription/admin-columns'
...
export function UserSubscriptionsDialog(props: Props) {
  ...
  const { linkMap, refetch: refetchLinks } = useAdminSubscriptionLinks(
    props.user?.id,
    props.open
  )
                    ...
                    subs.map((record) => {
                      ...
                      return (
                        <TableRow key={sub.id}>
                          ...
                          <AdminSubscriptionLinkBodyCells
                            link={linkMap.get(sub.id)}
                          />
                          <TableCell className='text-right'>
                            <div className='flex justify-end gap-1'>
                              <CancelStripeButton
                                link={linkMap.get(sub.id)}
                                onCanceled={refetchLinks}
                              />
                              <Button
                                size='sm'
                                variant='outline'
