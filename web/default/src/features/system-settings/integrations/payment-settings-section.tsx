...
import { DesktopRechargeToggle } from '@/transwork/recharge/desktop-recharge-toggle'
import { DesktopRechargeTiers } from '@/transwork/recharge/desktop-recharge-tiers'
import { DesktopSubscriptionToggle } from '@/transwork/subscription/desktop-subscription-toggle'
import { SubscriptionWebhookSecret } from '@/transwork/subscription/subscription-webhook-secret'
...
const paymentSchema = z.object({
  ...
  InviteCodeOptional: z.boolean(),
  FreeCreditForNewUser: z.coerce.number().min(0),
...
export function PaymentSettingsSection({
  ...
  const saveGeneralSettings = async () => {
    ...
    const sanitized = {
      ...
      InviteCodeOptional: values.InviteCodeOptional as boolean,
      FreeCreditForNewUser: values.FreeCreditForNewUser as number,
    ...
    const initial = {
      ...
      InviteCodeOptional: initialRef.current.InviteCodeOptional,
      FreeCreditForNewUser: initialRef.current.FreeCreditForNewUser,
    ...
    if (sanitized.InviteCodeOptional !== initial.InviteCodeOptional) {
      updates.push({
        key: 'InviteCodeOptional',
        value: sanitized.InviteCodeOptional,
      })
    }

    if (sanitized.FreeCreditForNewUser !== initial.FreeCreditForNewUser) {
      updates.push({
        key: 'FreeCreditForNewUser',
        value: sanitized.FreeCreditForNewUser,
      })
    }
      ...
      <Form {...form}>
        ...
        >
          <div className='space-y-4'>
              ...
              render={({ field }) => (
                <FormItem className='flex flex-row items-center justify-between rounded-lg border p-4'>
                  <div className='space-y-0.5'>
                    ...
                    <FormDescription>
                      {t(
                        'When on, new users can skip the invite code and receive free starter credit.'
                      )}
                    </FormDescription>
                  </div>
                  <FormControl>
            ...
            <FormField
              control={form.control}
              name='FreeCreditForNewUser'
              render={({ field }) => (
                <FormItem>
                  <FormLabel>{t('Free credit for new users')}</FormLabel>
                  <FormControl>
                    <Input
                      type='number'
                      min={0}
                  ...
                  <FormDescription>
                    {t(
                      'One-time starter credit granted to users who continue without an invite code.'
                    )}
                  </FormDescription>
                  <FormMessage />
                </FormItem>
            ...
            <DesktopRechargeToggle />
            <DesktopSubscriptionToggle />
            <DesktopRechargeTiers />
          </div>
