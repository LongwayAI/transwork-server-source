...
export function ChannelMutateDrawer({
  ...
  const currentTypeSupportedModels = useMemo(
    () => currentTypeConfig.supportedModels || [],
    [currentTypeConfig]
  )

  // Get basic models for the current channel type
  const basicModels = useMemo(() => {
    if (currentTypeSupportedModels.length > 0) {
      return currentTypeSupportedModels
    }
  ...
  }, [allModelsList, currentType, currentTypeSupportedModels])
  ...
  const modelOptions = useMemo(() => {
    const allModels = new Set([
      ...allModelsList,
      ...currentModelsArray,
      ...currentTypeSupportedModels,
    ])
    return Array.from(allModels).map((model) => ({
      value: model,
      label: model,
    }))
  }, [allModelsList, currentModelsArray, currentTypeSupportedModels])
  ...
  useEffect(() => {
    ...
    const defaultBaseUrl = currentTypeConfig.defaultBaseUrl
    if (defaultBaseUrl) {
  ...
  }, [currentType, currentTypeConfig, isEditing, form])
