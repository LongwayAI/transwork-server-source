...
import {
  getAdminColumns,
  loadLinkMap,
} from '../../../../transwork/subscription/adminColumns';
...
const UserSubscriptionsModal = ({ visible, onCancel, user, t, onSuccess }) => {
  ...
  const loadUserSubscriptions = async () => {
    ...
    try {
      ...
      if (res.data?.success) {
        ...
        setSubs(next);
        setLinkMap(await loadLinkMap(user.id));
        setCurrentPage(1);
      } else {
        showError(res.data?.message || t('加载失败'));
      }
  ...
  const columns = useMemo(() => {
    return [
      ...
      ...getAdminColumns({ t, linkMap, reload: loadUserSubscriptions }),
      {
        title: '',
        key: 'operate',
        width: 140,
