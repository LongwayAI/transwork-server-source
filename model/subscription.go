...
// GetPrimaryInPeriodSubscriptionsByUserIds returns, for each userId that has a
// subscription still within its paid period (end_time > now), the primary one —
// the most-recently-started (start_time desc, id desc). Users with no in-period
// subscription are absent from the returned map. Status is not filtered, so a
// cancelled-but-in-period subscription is still returned with its real status.
func GetPrimaryInPeriodSubscriptionsByUserIds(userIds []int) (map[int]*UserSubscription, error) {
	result := make(map[int]*UserSubscription)
	if len(userIds) == 0 {
		return result, nil
	}
	now := common.GetTimestamp()
	var subs []UserSubscription
	if err := DB.Where("user_id IN ? AND end_time > ?", userIds, now).
