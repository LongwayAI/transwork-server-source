...
func CompleteDesktopLogin(c *gin.Context) {
	...
	// Best-effort: a missing avatar must never block a sign-in.
	if err := upsertUserAvatar(user.Id, info.AvatarURL); err != nil {
		common.SysLog("desktop-login avatar persist failed: " + err.Error())
	}
