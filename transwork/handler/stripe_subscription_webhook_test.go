...
func TestMain(m *testing.M) {
	...
	// These four are plain globals that production writes once at startup and
	// then only reads. Several of those readers are fire-and-forget gopool
	// goroutines (the quota cache delta, the consume-log metrics sample) which
	// outlive the request — and therefore the test — that spawned them, so a
	// test that assigns to one races the previous test's stragglers. Setting
	// them here, before m.Run starts any goroutine, and never restoring them
	// keeps them write-once for the whole binary, matching production.
	common.RedisEnabled = false
	common.BatchUpdateEnabled = false
	// migrateDB only runs on the master node; tests need it to run.
	common.IsMasterNode = true
	// Every DB in this package is SQLite; model.InitDB would set this anyway.
...
func setupDB(t *testing.T) {
	...
	prev := model.DB
	model.DB = db
	// common.UsingSQLite is set once in TestMain; see the note there for why it
	// is not toggled per test.
	t.Cleanup(func() {
		model.DB = prev
	})
}
