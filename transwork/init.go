...
func Init() error {
	// Started before the migrations below, not after: each of them returns on
	// error, so an unrelated failure (Stripe links, waitlist, free credit,
	// avatars) would silently take the ASR leak monitor down with it for the
	// life of the process. main.go reports any error from this function under a
	// GCS-branded warning, so that failure would also point an operator at the
	// wrong subsystem. The monitor needs only model.DB and asr_reservations,
	// both ready before Init is called.
	StartAsrStaleReservationMonitor()
	...
	if err := model.DB.AutoMigrate(&twmodel.UserAvatar{}); err != nil {
		return err
	}
