...
// IsAnyOnlineTopUpEnabled reports whether at least one paid online top-up
// provider is fully configured. It is the canonical "is payment configured"
// check (gate 1 of desktop recharge). DRY over the per-provider helpers above
// so it can never drift from real payment readiness.
func IsAnyOnlineTopUpEnabled() bool {
	return isEpayTopUpEnabled() ||
