...
func TestIsAnyOnlineTopUpEnabled_DerivesFromProviderState(t *testing.T) {
	...
	if IsAnyOnlineTopUpEnabled() {
		t.Fatal("expected false when no online provider is configured")
	}

	setting.StripeApiSecret, setting.StripeWebhookSecret, setting.StripePriceId = "sk_test", "whsec_test", "price_123"
	if !IsAnyOnlineTopUpEnabled() {
		t.Fatal("expected true when Stripe is fully configured")
	}
}
