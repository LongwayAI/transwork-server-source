...
func genStripeLink(referenceId string, customerId string, email string, amount int64, successURL string, cancelURL string) (string, error) {
	...
	if successURL == "" {
		successURL = strings.TrimRight(system_setting.ServerAddress, "/") + "/topup/result?status=success"
	}
	if cancelURL == "" {
		cancelURL = strings.TrimRight(system_setting.ServerAddress, "/") + "/topup/result?status=cancel"
	}
