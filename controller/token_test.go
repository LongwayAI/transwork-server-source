...
func setupTokenControllerTestDB(t *testing.T) *gorm.DB {
	...
	seedTokenControllerTestUser(t, db, 1, "default")
	return db
}

// seedTokenControllerTestUser creates the acting user row. AddToken and
// UpdateToken validate the requested token group against this user's own group,
// so the handler tests need a real user to act as. Seeded only for the local
// SQLite fixture — the external MySQL/Postgres migration tests manage their own
// table lifecycle and must not have a users table created underneath them.
func seedTokenControllerTestUser(t *testing.T, db *gorm.DB, id int, group string) {
	t.Helper()

	if err := db.AutoMigrate(&model.User{}); err != nil {
		t.Fatalf("failed to migrate user table: %v", err)
	}
	// AffCode carries a unique index, so distinct users need distinct codes —
	// two rows both defaulting to "" collide.
	user := model.User{
		Id:       id,
		Username: "test-user-" + strconv.Itoa(id),
		Group:    group,
...
// A user must not be able to persist a token carrying a group they are not
// entitled to. middleware.TokenAuth applies the same check at relay time, so
// this was not exploitable, but storing it left the authorization decision
// resting entirely on that single downstream check — any future code path that
// trusted token.Group without re-validating would have made it live.
//
// "svip" is the group under test because it ships in the default group-ratio
// table but NOT in setting.userUsableGroups, so a "default" user genuinely
// cannot use it. Note "vip" would be the wrong choice: upstream ships it inside
// userUsableGroups, so every user may legitimately self-assign it.
func TestAddTokenRejectsGroupOutsideUserUsableGroups(t *testing.T) {
	...
	body := map[string]any{
		...
		"group":           "svip", // user's group is "default"
	}
	...
	response := decodeAPIResponse(t, recorder)
	if response.Success {
		t.Fatal("AddToken accepted a group the user cannot use; the escalation must " +
			"be refused at write time, not left for TokenAuth to catch at use time")
	}

	// Load rows rather than filtering on `group` in SQL — it is a reserved word
	// whose quoting differs across the three supported databases (Rule 2).
	var tokens []model.Token
	if err := model.DB.Find(&tokens).Error; err != nil {
		t.Fatalf("failed to load tokens: %v", err)
	}
	for _, stored := range tokens {
		if stored.Group == "svip" {
			t.Fatal("a token with an unusable group was persisted anyway")
		}
	}
}

// Being inside the user's usable groups is necessary but not sufficient:
// TokenAuth also rejects a group that has no entry in the ratio table
// (middleware/auth.go:391-396). A "vip" user's usable set includes the special
// group "append_1" (setting/ratio_setting/group_ratio.go:28-33), which is
// absent from the default ratio table — so without this second guard the token
// would save "successfully" and then 403 on every relay request, which is
// exactly the write-time/use-time mismatch this change exists to remove.
func TestAddTokenRejectsGroupWithNoRatioEntry(t *testing.T) {
	db := setupTokenControllerTestDB(t)
	seedTokenControllerTestUser(t, db, 2, "vip")

	body := map[string]any{
		"name":         "usable-but-unpriced",
		"expired_time": -1,
		"group":        "append_1",
	}

	ctx, recorder := newAuthenticatedContext(t, http.MethodPost, "/api/token/", body, 2)
	AddToken(ctx)

	response := decodeAPIResponse(t, recorder)
	if response.Success {
		t.Fatal("AddToken accepted a group with no ratio entry; TokenAuth would " +
			"reject it as deprecated on every request made with that token")
	}
}

// The same guard on the update path: without it, a user could create a token
// with an allowed group and immediately edit it to a privileged one, which
// would make the AddToken check pure theatre.
func TestUpdateTokenRejectsGroupOutsideUserUsableGroups(t *testing.T) {
	db := setupTokenControllerTestDB(t)
	token := seedToken(t, db, 1, "editable-token", "ghij1234klmn5678")
	...
	response := decodeAPIResponse(t, recorder)
	if response.Success {
		t.Fatal("UpdateToken accepted a group the user cannot use, so the AddToken " +
			"check can be bypassed with a follow-up edit")
	}
	...
	if stored.Group == "svip" {
		t.Fatal("the rejected group was written to the database anyway")
	}
}

// The legitimate case must keep working: a token in a group the user does hold
// is still accepted. Without this, the guard above could be "passed" by
// rejecting every group.
func TestUpdateTokenAllowsGroupWithinUserUsableGroups(t *testing.T) {
	db := setupTokenControllerTestDB(t)
	token := seedToken(t, db, 1, "editable-token", "opqr1234stuv5678")
	...
	body := map[string]any{
		...
		"remain_quota":    100,
		"unlimited_quota": true,
		"group":           "default", // the user's own group
	}
	...
	response := decodeAPIResponse(t, recorder)
	if !response.Success {
		t.Fatalf("a token in the user's own group must still be updatable, got: %s", response.Message)
	}
}
